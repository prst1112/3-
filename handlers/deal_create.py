...
async def enter_description(message: Message, state: FSMContext) -> None:
    ...
    amount_str = format_amount(data["amount"], data["currency"], lang, data.get("card_country"))
    ...
    if data["currency"] == "card":
        # Продавцу НЕ показываем его же привязанную карту как "куда платить" —
        # покупатель платит на нашу (платформы) карту, а не напрямую продавцу.
        # Привязанная карта нужна только чтобы определить валюту и потом,
        # когда появится реальный вывод, знать, куда переводить продавцу.
        text += "\n\n" + t(lang, "card_payment_note")
